...
def run_bulk_processing(base_df, lusha_key, apollo_key, progress_callback=None):
...
def render_bulk_processing():
    ...
    st.caption(
        "Upload CSV/XLSX containing Company and/or Domain. Domain-first rows go directly to Apollo; "
        "company-only rows use Lusha only for company resolution. No Lusha person/prospecting calls are made. "
        "Technology is fetched from Apollo when available."
    )

    template = pd.DataFrame({
        "Company": ["Denave", "Motherhood Hospital"],
        "Domain": ["denave.com", "motherhoodhospital.com"],
    })
    st.download_button(
    ...
    uploaded = st.file_uploader(
        ...
        help="Recommended columns: Company and Domain. If Domain is present, Lusha is skipped for that row.",
    )
    ...
    domain_count = int((base_df["Input Domain"].astype(str).str.strip() != "").sum())
    company_only = len(base_df) - domain_count

    a, b, c = st.columns(3)
    a.metric("Companies", f"{len(base_df):,}")
    b.metric("With Domain", f"{domain_count:,}")
    c.metric("Need Lusha Resolution", f"{company_only:,}")

    st.warning(
        "Credit safety: this bulk mode NEVER fetches Lusha people. "
        "Apollo Organization Enrichment is paid per company. If technology data is absent from that response, "
        "the app may make one additional Complete Organization Info call for that company, so bulk technology "
        "enrichment can use up to 2 Apollo credits/company."
    )

    if "bulk_results" not in st.session_state:
        st.session_state["bulk_results"] = None
    if "bulk_file_signature" not in st.session_state:
        st.session_state["bulk_file_signature"] = None

    current_signature = f"{uploaded.name}:{len(base_df)}:{','.join(base_df['Input Domain'].astype(str).tolist()[:10])}"
    ...
    if not can_run:
        st.error("Enter your Apollo API key in the sidebar before starting bulk processing.")

    if st.button("🚀 Process All Companies", type="primary", use_container_width=True, disabled=not can_run):
        progress = st.progress(0, text="Starting bulk processing...")
        status_box = st.empty()

        def update_progress(done, total):
            pct = int(done / total * 100) if total else 100
            progress.progress(pct, text=f"Processing {done:,} / {total:,} companies")
